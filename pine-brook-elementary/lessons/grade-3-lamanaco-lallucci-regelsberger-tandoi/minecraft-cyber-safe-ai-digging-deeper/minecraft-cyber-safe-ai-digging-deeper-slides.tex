...
\usepackage{tikz}
\usepackage{hyperref}
\definecolor{pbprimary}{HTML}{9B5DE5}
\definecolor{pbsecondary}{HTML}{F1E7FF}
...
\begin{frame}[plain]
\begin{center}
{\usebeamerfont{title}\color{pbprimary}\Huge Minecraft Cyber Safe: AI Digging Deeper\par}
\vspace{0.3cm}
...
\node[anchor=west, text width=10.2cm] at (0.35,0.68) {\large Grade 3 learners: big steps, clear checks, and one thing at a time.};
\end{tikzpicture}
\end{center}
\end{frame}
...
\end{frame}
\begin{frame}{Step 1}
\begin{block}{Open Minecraft Education and get to the correct world.}
Open Minecraft Education and get to the correct world.
\end{block}
\begin{itemize}
\item Use the click-path on the slide. If login breaks, pause and use the help routine.
\item Project the exact click path before students begin so setup friction does not consume the lesson.
\end{itemize}
\centering\large\color{pbprimary} Pause. Check. Then move on.
\end{frame}
\begin{frame}{Step 2}
\begin{block}{Read the challenge and build your code a little at a time.}
Read the challenge and build your code a little at a time.
\end{block}
\begin{itemize}
\item Test after small changes so you can see what worked.
\item Use a stop-and-check slide after world entry so students confirm they are in the correct place before coding.
\end{itemize}
\centering\large\color{pbprimary} Pause. Check. Then move on.
\end{frame}
\begin{frame}{Step 3}
\begin{block}{Debug with a partner and improve your solution.}
Debug with a partner and improve your solution.
\end{block}
\begin{itemize}
\item Say what changed, why you changed it, and what happened next.
\item Require test-after-each-change routines so debugging stays visible and manageable.
\end{itemize}
\centering\large\color{pbprimary} Pause. Check. Then move on.
\end{frame}
...
\item Sign-in or menu navigation can block learning before coding even starts.
\end{enumerate}
...
\footnotesize
\begin{itemize}
\item Today we are working on minecraft cyber safe: ai digging deeper. Watch first, then try it with me.
