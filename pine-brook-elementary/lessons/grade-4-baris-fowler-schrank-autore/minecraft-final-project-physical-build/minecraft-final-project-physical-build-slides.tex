...
\usepackage{tikz}
\usepackage{hyperref}
\definecolor{pbprimary}{HTML}{6C5CE7}
\definecolor{pbsecondary}{HTML}{EAE5FF}
...
\title{Minecraft Final Project: Physical Build}
...
\begin{frame}[plain]
\begin{center}
{\usebeamerfont{title}\color{pbprimary}\Huge Minecraft Final Project: Physical Build\par}
\vspace{0.3cm}
...
\node[anchor=west, text width=10.2cm] at (0.35,0.68) {\large Grade 4 learners: big steps, clear checks, and one thing at a time.};
\end{tikzpicture}
\end{center}
\end{frame}
...
\footnotesize
\begin{itemize}
\item Today we are working on minecraft final project: physical build. Watch first, then try it with me.
