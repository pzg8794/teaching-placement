# 🟢 Flex Day - Complete Previous Assignments | Option: | STEM Wheels Challenges Lessons

**Color Code:** 🟢 Lessons

> **📌 What This Is:** Workbook-backed lesson packet for a real IGNITE lesson under this grade.
> **🧭 Start Here When:** You need the lesson expectation, local evidence links, official references, and next-step improvement moves in one place.
> **🎯 Main Goal:** Use the school planning goals plus inclusion to document what this lesson is supposed to do, what evidence exists, and what needs to be improved next.

---

## ⚡ Quick Scan

- **Grade:** Grade 2
- **Grade hub:** [open grade hub](../grade-2-notes.md)
- **Workbook note:** [open workbook note](../../../teaching-placement-ignite-curriculum-gcsd-25-26-grade-2-notes.md)
- **Schedule reference:** [spring-2026-teaching-schedule-derek.md](../../../spring-2026-teaching-schedule-derek.md)
- **Workbook sessions:** `Session 8, Session 14, Session 28`
- **Months:** October, December, April
- **Class blocks:** Day 1 `10:45-11:35` Callon, Day 1 `2:15-3:05` Henchen, Day 4 `10:45-11:35` Barthelman
- **LaTeX source:** [flex-day-complete-previous-assignments-option-stem-wheels-challenges-les.tex](./flex-day-complete-previous-assignments-option-stem-wheels-challenges-les.tex)
- **Bib file:** [flex-day-complete-previous-assignments-option-stem-wheels-challenges-les.bib](./flex-day-complete-previous-assignments-option-stem-wheels-challenges-les.bib)
- **Standards addressed:** 2-3.CT.10
- **Materials / resources:** Design Worksheet | Building Materials: | Strawbees | Plus Plus Blocks | Zoob | Keeva Planks
- **Learning targets:** "I can design and build a contraption that will meet the requirements of the assignment and fit in the required work area. | | I can rethink my ideas to improve my design. | | I can communicate my ideas. | | I can compromise or try my partners ideas. | | I can be supportive of my partner by collaborating with their design ideas. | | I can sense my feelings and manage my feelings as I work.
- **Special education considerations:** Model using manipulatives. Provide visuals. Utilize adult support to work with students. Focus on functional communication. Give students time to explore materials.
- **Cross-curricular context:** 2 - ARC: Words Change the World (Literacy Lab) | 6-8 weeks | 2 - Science: Save the Bees (October-November) | 2 - Math: Numbers Within 100: Addition, Subtraction, Time, and Money | (Oct 30 - Dec 22); 2 - ARC: Bugs in their Ecosystems (Informational) | Weeks | 2 - Science: Save the Bees (October-November) | 2 - Math: Numbers Within 100: Addition, Subtraction, Time, and Money | (Oct 30 - Dec 22); 2 - ARC: Animal Stories and Animals (Narrative) | 9 Weeks | 2 - Science: Made of Matter (End of Jan. - Feb.) | 2 - Math: Length: Measurement, Addition, and Subtraction and Line Plots | (Mar 4 - May 3)

## 🔗 Transcript and Local Evidence

- **Transcript:** [260227-stem-routines-and-dismissal-protocols.md](../../../../transcripts/non-lesson-context/260227-stem-routines-and-dismissal-protocols.md)

## 🌐 Official Lesson / Reference

- [CSTA K-12 Standards](https://csteachers.org/k12standards/interactive/)

## 🛠️ Materials and Teaching Moves

- Begin by naming the lesson purpose and the success condition in plain language.
- Model the first step before independent or partner work starts.
- Keep one visible checklist, anchor chart, or projected example available throughout the lesson.
- Pause once mid-lesson for a quick debug, reflection, or reset point.
- Close with one short proof task: explain, show, or save what was completed.

## ✅ Assessment and Proof

- Student completes the core task or an accessible version of it.
- Student can explain at least one step, pattern, choice, or fix.
- Teacher captures one proof item: screenshot, observation note, exit response, or student artifact.

## 📌 Next Revision Moves

- Add stronger transcript evidence when a direct class recording exists.
- Save one student example in this lesson folder after reteaching.
- Tighten the alignment between the workbook target and the actual product students create.
