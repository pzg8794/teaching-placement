...
\title{Getting Started with Book Creator}
...
\vspace{0.35cm}
\begin{center}
{\Huge\bfseries\color{ink} Getting Started with Book Creator\par}
\vspace{0.2cm}
{\Large\color{coral} Look. Try. Share.\par}
\vspace{0.35cm}
...
\node[rounded corners=18pt, fill=skyblue!18, draw=skyblue!60, line width=1.2pt,
      ...
      {\Large\bfseries Big visuals, one direction at a time, and lots of chances to point, move, and show.};
\end{tikzpicture}
...
\end{tikzpicture}
\end{center}
\note{Today we are working on getting started with book}
\end{frame}
...
\Large K-1.IC.1: I can identify how books can be read and created both with and without computing devices.
\end{block}
\vspace{0.15cm}
{\large\bfseries\color{coral} Success looks like this:}
\begin{itemize}\large
\item K-1.IC.1: I can identify how books can be read and created both with and without computing devices.
\item K-1.CY.2: I can explain how my accounts (chromebook, Book Creator Library, etc.) are kept secure.
\item K-1.CY.4: I can decode words using a simple code.
\end{itemize}
\column{0.4\textwidth}
...
\node[align=center, text width=3.3cm] at (2.45,3.45) {\large\bfseries Kindergarten};
\draw[rounded corners=10pt, fill=white, draw=coral!70, line width=1pt] (0.45,1.4) rectangle (4.4,2.45);
\node[align=center, text width=3.3cm] at (2.45,1.93) {\small\bfseries Early elementary learners who benefit from visual modeling, predictable routines, and very small chunks of task time.};
\draw[rounded corners=10pt, fill=white, draw=leafgreen!70!black, line width=1pt] (0.45,0.25) rectangle (4.4,1.05);
\node[align=center, text width=3.3cm] at (2.45,0.65) {\scriptsize\bfseries Day 1 `12:25-1:15` Justinger, Day 2 `12:25-1:15` Kesys, Day 4 `12:25-1:15` Pum};
\end{tikzpicture}
\end{columns}
\note{Today we are working on getting started with book}
\end{frame}
...
{\Huge\bfseries 1\\[6pt]\Large Model the first screen and the exact};
...
{\Huge\bfseries 2\\[6pt]\Large Use sentence frames or model examples when};
...
\begin{itemize}\large
\item Tool navigation can overshadow the content goal if too many options
\item Directions can feel hidden
