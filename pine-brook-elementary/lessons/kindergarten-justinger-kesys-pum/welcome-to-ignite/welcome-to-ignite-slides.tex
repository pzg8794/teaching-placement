...
\end{tikzpicture}
\end{center}
\note{Today we are working on welcome to ignite}
\end{frame}
...
\Large We can collect data about material choices (K-1.CT.2)
\end{block}
\vspace{0.15cm}
{\large\bfseries\color{coral} Success looks like this:}
\begin{itemize}\large
\item We can collect data about material choices (K-1.CT.2)
\item We can create a visual chart of the data we collect (K-1.CT.3)
\item I can build my name by breaking the task down into smaller parts (K-1.CT.4)
\end{itemize}
\column{0.4\textwidth}
