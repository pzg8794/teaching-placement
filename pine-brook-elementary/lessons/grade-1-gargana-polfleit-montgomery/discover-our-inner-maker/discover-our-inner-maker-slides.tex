...
\usepackage{tikz}
\usepackage{hyperref}
\definecolor{pbprimary}{HTML}{2A9D8F}
\definecolor{pbsecondary}{HTML}{DDF5EF}
...
\begin{frame}[plain]
\begin{center}
{\usebeamerfont{title}\color{pbprimary}\Huge Discover Our Inner Maker\par}
\vspace{0.3cm}
...
\node[anchor=west, text width=10.2cm] at (0.35,0.68) {\large Grade 1 learners: big steps, clear checks, and one thing at a time.};
\end{tikzpicture}
\end{center}
\end{frame}
\begin{frame}{Todays Goal}
\begin{itemize}
\item "Be a Maker" by Katey Howes
\item Chart paper or whiteboard
\item Markers
\end{itemize}
...
\footnotesize
\begin{itemize}
\item Today we are working on discover our inner maker. Watch first, then try it with me.
