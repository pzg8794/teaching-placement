# 🟦 Event-Driven Robot Coding

**Color Code:** 🟢 Lessons

> **📌 What This Is:** A grade-organized lesson packet connected to the schedule, transcript lesson notes, and a formal online starting point.
> **🧭 Start Here When:** You know the grade or class block and want the strongest reusable version of the lesson.
> **👀 Best Use:** Read the quick scan, confirm the class match, then jump into the transcript lesson notes or LaTeX source.

---

## ⚡ Quick Scan

- 👥 **Grade:** Grade 4
- 📘 **Grade Hub:** [Grade 4 Notes](../grade-4-Notes.md)
- 📅 **Schedule Reference:** [Spring2026_TeachingSchedule_Derek.md](../../school/Spring2026_TeachingSchedule_Derek.md)
- 🗂️ **Workbook Sheet:** `4th` in `Teaching_Placement-IGNITE Curriculum_ GCSD25-26.xlsx`
- 🎯 **Lesson Focus:** Robotics with events, sensors, reactions, and challenge-based troubleshooting.
- 🧾 **LaTeX Source:** [event-driven-robot-coding.tex](./event-driven-robot-coding.tex)
- 📚 **Bib File:** [event-driven-robot-coding.bib](./event-driven-robot-coding.bib)
- 🌐 **Formal Starting Point:** [Sphero Edu K-12 Teacher Resource Guide](https://dmmedia.sphero.com/email-marketing/Sphero-Edu/SpheroEdu-k12-teacher-resource-guide-v1_updated050818.pdf)

---

## 📍 Likely Class Match

- Strong fit for Grade 4 blocks when the transcript mentions events, sensors, flashlight tag, or ambient light reactions.
- 🕒 **Likely grade blocks:** Day 1 · 1:15-2:10 · Baris, Day 2 · 1:15-2:10 · Fowler, Day 3 · 1:15-2:10 · Schrank, Day 4 · 1:15-2:10 · Autore

## 🎯 Learning Targets

- Students build an event-driven or sensor-based program.
- Students test and revise code based on robot behavior.
- Students explain the trigger and response in their program.

## 🛠️ Materials

- Robots
- Student devices
- Challenge setup or obstacle space

## ⚪ Transcript Lesson Notes

- ⚪ [2026 03 03 Sphero Event Driven Block Coding With Ambient Light Sensors And Flashlight Tag](../../transcript-derived-lessons/260303-grade-4-baris-event-sensors.md)
- ⚪ [2026 03 12 Bb 8 Programming Puzzle And Classroom Management](../../transcript-derived-lessons/260312-grade-3-lamanaco-bb8.md)

## 🔗 Related Local Materials

- 🔗 No direct local lesson packet linked yet.

## 🌐 Formal / Online Starting Point

- 🌍 [Sphero Edu K-12 Teacher Resource Guide](https://dmmedia.sphero.com/email-marketing/Sphero-Edu/SpheroEdu-k12-teacher-resource-guide-v1_updated050818.pdf)

## 🔎 Search Titles

- 🔎 `event driven robot coding elementary`
- 🔎 `Sphero sensor flashlight lesson`

## ✅ Next Steps

- ✅ Use the transcript lesson notes above to enrich timing, teacher language, and classroom-management moves.
- ✅ Use the schedule and workbook sheet to confirm which class block this version best fits.
- ✅ Store compiled PDFs, slide decks, screenshots, or handouts in [artifacts/](./artifacts/).
