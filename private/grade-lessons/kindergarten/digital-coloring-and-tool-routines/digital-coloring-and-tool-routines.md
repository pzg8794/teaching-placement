# ⬜ Digital Coloring and Tool Routines

**Color Code:** 🟢 Lessons

> **📌 What This Is:** A grade-organized lesson packet connected to the schedule, transcript lesson notes, and a formal online starting point.
> **🧭 Start Here When:** You know the grade or class block and want the strongest reusable version of the lesson.
> **👀 Best Use:** Read the quick scan, confirm the class match, then jump into the transcript lesson notes or LaTeX source.

---

## ⚡ Quick Scan

- 👥 **Grade:** Kindergarten
- 📘 **Grade Hub:** [Kindergarten Notes](../kindergarten-Notes.md)
- 📅 **Schedule Reference:** [Spring2026_TeachingSchedule_Derek.md](../../school/Spring2026_TeachingSchedule_Derek.md)
- 🗂️ **Workbook Sheet:** `K` in `Teaching_Placement-IGNITE Curriculum_ GCSD25-26.xlsx`
- 🎯 **Lesson Focus:** Device routines, calm transitions, and simple digital creativity.
- 🧾 **LaTeX Source:** [digital-coloring-and-tool-routines.tex](./digital-coloring-and-tool-routines.tex)
- 📚 **Bib File:** [digital-coloring-and-tool-routines.bib](./digital-coloring-and-tool-routines.bib)
- 🌐 **Formal Starting Point:** [Seesaw creative activity ideas](https://web.seesaw.me/)

---

## 📍 Likely Class Match

- Best fit for the Kindergarten blocks when the transcript mentions coloring, routines, or first-week tool use.
- 🕒 **Likely grade blocks:** Day 1 · 12:25-1:15 · Justinger, Day 2 · 12:25-1:15 · Kesys, Day 4 · 12:25-1:15 · Pum

## 🎯 Learning Targets

- Students open and use a digital coloring tool with minimal support.
- Students follow device-handling and transition routines safely.
- Students describe one tool or action they used during the task.

## 🛠️ Materials

- Student devices
- Digital coloring app or site
- Projected model example

## ⚪ Transcript Lesson Notes

- ⚪ [2026 03 18 Digital Coloring And Introduction To Robotics](../../transcript-derived-lessons/2026-03-18-lecture-digital-coloring-and-introduction-to-robotics.md)

## 🔗 Related Local Materials

- 🔗 No direct local lesson packet linked yet.

## 🌐 Formal / Online Starting Point

- 🌍 [Seesaw creative activity ideas](https://web.seesaw.me/)

## 🔎 Search Titles

- 🔎 `Seesaw digital coloring activity kindergarten`
- 🔎 `Kindergarten technology routines coloring lesson`

## ✅ Next Steps

- ✅ Use the transcript lesson notes above to enrich timing, teacher language, and classroom-management moves.
- ✅ Use the schedule and workbook sheet to confirm which class block this version best fits.
- ✅ Store compiled PDFs, slide decks, screenshots, or handouts in [artifacts/](./artifacts/).
