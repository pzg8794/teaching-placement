# 🟦 Grade 4 Lessons Hub

**Color Code:** 🟢 Lessons

> **📌 What This Is:** Grade 4 packets focused on strongest current CS instruction in the placement: Minecraft coding and event-driven robotics.
> **🧭 Start Here When:** You know the grade first and want the strongest lesson packet to open next.
> **👀 Best Use:** Open a lesson packet, read `Quick Scan`, then jump to `Evidence / Proof of Instruction` and `UDL and Inclusion Supports`.

---

## 📍 Start Here

- ⛏️ [Minecraft Coding Fundamentals](./minecraft-coding-fundamentals/minecraft-coding-fundamentals.md)  
  Pilot-quality CS packet with standards, transcript proof, and inclusion guidance.
- 🔦 [Event-Driven Robot Coding](./event-driven-robot-coding/event-driven-robot-coding.md)  
  Strong CS packet for events, sensors, and debugging.

## 📚 Shared Reference Links

- ⚪ [Transcript-derived lessons index](../lessons/evidence/Evidence-Notes.md)
- ⚪ [Source transcripts index](../transcripts/Transcripts-Notes.md)
- 📅 [Teaching schedule](../school/Spring2026_TeachingSchedule_Derek.md)
- 📏 [CSTA K-12 Standards](https://csteachers.org/k12standards/interactive/)

## ✅ What To Keep Improving

- ✅ Strengthen each packet’s standards alignment with the paired `.tex` and `.bib` files next.
- ✅ Keep storing real evidence in this lesson folder, not only compiled PDFs.
- ✅ Treat transcript evidence as the proof spine for what was actually taught.
