\documentclass[11pt]{article}
    \usepackage[utf8]{inputenc}
    \usepackage[T1]{fontenc}
    \usepackage[margin=1in]{geometry}
    \usepackage{hyperref}
    \usepackage{enumitem}
    \title{Minecraft Cyber Safe: AI Digging Deeper}
    \author{Piter Garcia}
    \date{March 20, 2026}
    \begin{document}
    \maketitle
    \section*{Quick Scan}
    \begin{itemize}[leftmargin=*]
    \item Grade: Grade 3
    \item Workbook sessions: Session 11
    \item Class blocks: Day 3 9:50-10:40 Lamanaco, Day 4 9:50-10:40 Lallucci, Day 5 9:50-10:40 Regelsberger, Day 5 2:15-3:05 Tandoi
    \item Reference file: minecraft-cyber-safe-ai-digging-deeper.bib
    \end{itemize}
    \section*{School Planning Goals and Inclusion}
    \begin{itemize}[leftmargin=*]
    \item What is expected: I can identify how computing technologies, such as AI have changed the way people live and work (2-3.IC.1) | I can recognize that the rules around computing devices can change in different settings, for example from IGNITE to the classroom (2-3.IC.2) | I understand why information should be kept private (2-3.CY.1) and know ways to keep my personal information safe (2-3.CY.2)
    \item What we achieved: No direct transcript match is attached yet. Treat this lesson as workbook-backed and enrich it when a matching classroom transcript is available.
    \item What needs improvement: stronger proof, cleaner assessment notes, and tighter lesson artifacts.
    \item How to improve: add transcript proof, one saved artifact, and clearer success criteria.
    \item Inclusion focus: use visual modeling, chunked directions, predictable routines, and flexible participation roles.
    \end{itemize}
    \section*{Official References}
    \begin{itemize}[leftmargin=*]
    \item \url{https://csteachers.org/k12standards/interactive/}
\item \url{https://education.minecraft.net/en-us/resources/computer-science/coding-with-minecraft}
\item \url{https://code.org/hour-of-ai}
\item \url{https://www.commonsense.org/education/digital-citizenship}

    \end{itemize}
    \end{document}
